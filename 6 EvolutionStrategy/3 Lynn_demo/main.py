...
class ES(object):
    ...
    def run(self):
        ...
        for g in range(self.N_GENERATION):
            ...
            net_r = self.get_reward(net_shapes, net_params, env, CONFIG['ep_max_step'], CONFIG['continuous_a'], None, )
            ...
            if mar >= CONFIG['eval_threshold']:
                p = self.params_reshape(net_shapes, net_params)
                np.save("ES_Net/model.npy", p)
                break

    def display(self, CONFIG):
        # testnet/
        print("\nTESTING....")
        p = np.load("ES_Net/model.npy")
        while True:
            s = env.reset()
    ...
    def train(self, net_shapes, net_params, optimizer, utility, pool):
        ...
        jobs = [
            pool.apply_async(self.get_reward, (net_shapes, net_params, env, CONFIG['ep_max_step'], CONFIG['continuous_a'],
