...
class ES(object):
    def __init__(self):
        ...
        self.base = self.N_KID * 2  # *2 for mirrored sampling
        self.rank = np.arange(1, self.base + 1)
        self.util_ = np.maximum(0, np.log(self.base / 2 + 1) - np.log(self.rank))
        self.utility = self.util_ / self.util_.sum() - 1 / self.base  # 根据排名选择更新力度

        # training
        net_shapes, net_params = self.build_net()
        self.env = gym.make(self.CONFIG['game']).unwrapped
        optimizer = SGD(net_params, self.LR)
        pool = mp.Pool(processes=self.N_CORE)
        mar = None  # moving average reward
        for g in range(self.N_GENERATION):
            t0 = time.time()
            net_params, kid_rewards = self.train(net_shapes, net_params, optimizer, self.utility, pool)

            # test trained net without noise
            net_r = self.get_reward(net_shapes, net_params, self.env, self.CONFIG['ep_max_step'], self.CONFIG['continuous_a'], None, )
            ...
            if mar >= self.CONFIG['eval_threshold']: break
    ...
    def get_reward(self, shapes, params, env, ep_max_step, continuous_a, seed_and_id=None, ):
        # perturb parameters using seed
        if seed_and_id is not None:
            seed, k_id = seed_and_id
            np.random.seed(seed)
            params += self.sign(k_id) * self.SIGMA * np.random.randn(params.size)
    ...
    def train(self, net_shapes, net_params, optimizer, utility, pool):
        ...
        jobs = [
            pool.apply_async(self.get_reward, (net_shapes, net_params, self.env, self.CONFIG['ep_max_step'], self.CONFIG['continuous_a'],
        ...
        for ui, k_id in enumerate(kids_rank):
            ...
            cumulative_update += utility[ui] * self.sign(k_id) * np.random.randn(net_params.size)
